...
export default function WhatsAppWidget({ whatsappNumber }: { whatsappNumber: string }) {
  ...
  return (
    ...
    >
      <div className="absolute right-full mr-4 hidden whitespace-nowrap rounded-lg bg-[#121212] border border-white/10 px-4 py-2 text-sm font-medium text-white opacity-0 shadow-xl transition-all duration-300 group-hover:block group-hover:opacity-100 lg:block">
        Chat with us!
        {/* Little triangle arrow pointing right */}
        <div className="absolute top-1/2 -right-1.5 -translate-y-1/2 border-y-4 border-l-4 border-y-transparent border-l-white/10"></div>
        <div className="absolute top-1/2 -right-1 -translate-y-1/2 border-y-[3px] border-l-[3px] border-y-transparent border-l-[#121212]"></div>
      </div>
