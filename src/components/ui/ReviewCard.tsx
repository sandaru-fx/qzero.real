...
export default function ReviewCard({
  ...
  return (
    <motion.article
      ...
      className={`group/card relative min-h-[280px] cursor-pointer overflow-hidden rounded-2xl border border-white/20 bg-black/55 p-8 shadow-[0_24px_60px_rgba(0,0,0,0.55)] backdrop-blur-md transition-[border-color,box-shadow,background-color] duration-350 ease-[cubic-bezier(0.22,1,0.36,1)] hover:border-brand-gold/70 hover:bg-black/65 hover:shadow-[0_28px_70px_rgba(0,0,0,0.65)] sm:min-h-[320px] sm:p-10 ${
    ...
    >
      ...
      <div className="relative flex items-start gap-4 sm:gap-5">
        <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-full border border-white/25 bg-black shadow-[0_8px_24px_rgba(0,0,0,0.45)] sm:h-[4.5rem] sm:w-[4.5rem]">
          {review.imageUrl ? (
            <Image
              src={review.imageUrl}
        ...
        <div className="min-w-0 flex-1 pt-0.5">
          ...
          <p className="mt-1.5 truncate text-sm font-semibold tracking-wide text-brand-gold sm:text-base">
            {review.vehicleName}
          </p>
        </div>
      </div>
      ...
      <p className="relative mt-5 text-lg font-medium leading-[1.75] text-white sm:text-xl sm:leading-[1.8]">
        “{review.reviewText}”
      </p>
    </motion.article>
  );
